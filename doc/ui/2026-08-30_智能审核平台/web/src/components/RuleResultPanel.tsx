...
import { displayScore, PASS_MODE_LABEL, rulePassed } from '../mock/engine';
  ...
  return (
    <Space direction="vertical" size="middle" style={{ width: '100%' }}>
      <Descriptions size="small" bordered column={3}>
        ...
        <Descriptions.Item label="总分">{task.totalScore ?? '（本模式不汇总总分）'}</Descriptions.Item>
        <Descriptions.Item label="结果">
          <PassTag passed={task.passed} complete={task.complete} />
        </Descriptions.Item>
      </Descriptions>
      {!task.complete ? <Alert type="warning" showIcon message="存在机评失败的规则，结果标记为不完整。全部通过或红线条失败时整单不得判通过。" /> : null}
      <Table
        rowKey="ruleId"
        size="small"
        pagination={false}
